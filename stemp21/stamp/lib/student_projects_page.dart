  ...
  Future<void> _downloadEvaluationReport(
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        filePath =
            "${docsDir.path}/${widget.studentName}_${projectTitle.replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}.pdf";
      } else {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("✅ PDF saved in Documents!\n$filePath"),
  ...
  Future<void> _downloadAllProjectsReport() async {
    try {
      ...
      if (Platform.isAndroid) {
        ...
        filePath =
            "${docsDir.path}/${widget.studentName}_ALL_PROJECTS_${DateTime.now().millisecondsSinceEpoch}.pdf";
      } else {
        final dir = await getApplicationDocumentsDirectory();
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("✅ All projects PDF saved!\n$filePath"),
            duration: const Duration(seconds: 6),
          ),
        );
      }
